...
_SVC_COLORS = [
    "#f59e0b","#06b6d4","#8b5cf6","#10b981","#f97316",
    "#ec4899","#14b8a6","#6366f1","#84cc16","#ef4444",
    "#0ea5e9","#a78bfa","#fb923c","#34d399","#f43f5e",
]
...
def _sev_color(sev: str) -> str:
    return {"critical": "#ef4444", "high": "#ef4444",
            "warning": "#f59e0b", "medium": "#f59e0b",
            "info": "#3b82f6", "low": "#3b82f6"}.get(sev, "#71717a")

def _sev_bg(sev: str) -> str:
    return {"critical": "rgba(239,68,68,.10)", "high": "rgba(239,68,68,.10)",
            "warning": "rgba(245,158,11,.10)", "medium": "rgba(245,158,11,.10)",
            "info": "rgba(59,130,246,.10)", "low": "rgba(59,130,246,.10)"}.get(sev, "rgba(113,113,122,.10)")

def _sev_label(sev: str) -> str:
    return {"critical": "High", "high": "High",
            "warning": "Medium", "medium": "Medium",
            "info": "Low", "low": "Low"}.get(sev, sev.title())
...
def _status_dot(status: str) -> str:
    c = {"healthy": "#10b981", "warning": "#f59e0b", "critical": "#ef4444",
         "ok": "#10b981"}.get(status, "#71717a")
    label = {"healthy": "Healthy", "warning": "Warning",
             "critical": "Critical", "ok": "OK"}.get(status, status.title())
    return (f'<span style="display:inline-flex;align-items:center;gap:5px;font-size:12px;font-weight:600;color:{c}">'
...
def _finding_rows(f: Dict, idx: int) -> str:
    ...
    if is_group:
        gid     = f"grp-{_GROUP_ROW_IDX}"; _GROUP_ROW_IDX += 1
...
def _services_section(trend_data: Optional[Dict]) -> str:
    ...
    period_label = {"3d":"3 Days","1w":"7 Days","1m":"30 Days","3m":"3 Months","1y":"1 Year"}.get(period, period)
    ...
    for idx, svc in enumerate(by_service[:20]):
        ...
        if len(values) >= 2:
            ...
            trend_html = f'<span style="color:{clr};font-weight:600;font-size:12px">{arrow}{abs(delta):.1f}%</span>'
...
def _infrastructure_section(infra_data: Optional[Dict]) -> str:
    ...
    if ec2:
        ...
        cards_html += _infra_card("🖥️", "EC2 Compute", ec2.get("status","ok"),
                                  ec2.get("monthly_cost",0), rows, warning)
    ...
    if rds:
        rows = (
            ...
            + _infra_kv("Connections", f"{rds.get('connections_active',0):,} / {rds.get('connections_max',0):,}")
            + _infra_kv("Storage", f"{rds.get('storage_used_gb',0)} / {rds.get('storage_allocated_gb',0)} GB")
        )
        cards_html += _infra_card("🗄️", "RDS Databases", rds.get("status","ok"),
                                  rds.get("monthly_cost",0), rows, rds.get("warning") or rds.get("detail",""))
    ...
    if eks:
        ...
        cards_html += _infra_card("☸️", "EKS Kubernetes", eks.get("status","ok"),
                                  eks.get("monthly_cost",0), rows, eks.get("detail",""))
    ...
    if ec:
        ...
        cards_html += _infra_card("⚡", "ElastiCache / Redis", ec.get("status","ok"),
                                  ec.get("monthly_cost",0), rows, ec.get("detail",""))
    ...
    if os_:
        ...
        cards_html += _infra_card("🔍", "OpenSearch", os_.get("status","ok"),
                                  os_.get("monthly_cost",0), rows, os_.get("detail",""))
    ...
    if s3:
        ...
        cards_html += _infra_card("🪣", "S3 Storage", s3.get("status","ok"),
                                  s3.get("monthly_cost",0), rows, s3.get("detail",""))
    ...
    if cf:
        ...
        cards_html += _infra_card("🌐", "CloudFront CDN", cf.get("status","ok"),
                                  cf.get("monthly_cost",0), rows, cf.get("detail",""))
    ...
    if msk:
        ...
        cards_html += _infra_card("📨", "MSK Kafka", msk.get("status","ok"),
                                  msk.get("monthly_cost",0), rows, msk.get("detail",""))
...
def generate_html_report(
    ...
    insight_filtered = [
        i for i in insights
        if i.get("category") not in _SKIP_CATEGORIES
        and i.get("status") != "ok"
    ]

    # ── Split cleanup vs rightsize ───────────────────────────────────────────
    cleanup   = _deduplicate_findings([f for f in filtered if f.get("category") == "cleanup"])
    rightsize = _deduplicate_findings([f for f in filtered if f.get("category") in ("rightsize","rightsizing")])
    ...
    # ── Top opportunities ────────────────────────────────────────────────────
    top3 = sorted(filtered + [
        {"title": i["title"], "estimated_savings_usd": i.get("savings_usd",0),
         "severity": i.get("status","info"), "resource_type": "insight"}
        for i in insight_filtered if i.get("savings_usd",0) > 0
    ], key=lambda x: -x.get("estimated_savings_usd",0))[:3]
    ...
    if services_section:  toc_items.append(("📈", "Cost by Service"))
    if infra_section:     toc_items.append(("🏗️", "Infrastructure Health"))
    if cleanup_section:   toc_items.append(("🗑️", f"Cleanup ({len(cleanup)} findings)"))
    if rightsize_section: toc_items.append(("⚡", f"Rightsizing ({len(rightsize)} findings)"))
    if insights_section:  toc_items.append(("📊", f"Billing Insights ({len(insight_filtered)})"))
    if optimizer_section: toc_items.append(("🎯", "Cost Optimizer"))
