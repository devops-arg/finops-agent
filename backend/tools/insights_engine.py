...
def _ce_total(ce, start: str, end: str, filters: dict = None) -> float:
    kw = dict(TimePeriod={"Start": start, "End": end}, Granularity="MONTHLY", Metrics=["UnblendedCost"])
    if filters:
        kw["Filter"] = filters
...
def _ce_by_service(ce, start: str, end: str) -> list[dict]:
    ...
    for r in resp.get("ResultsByTime", []):
        for g in r.get("Groups", []):
            out.append({"service": g["Keys"][0], "cost": float(g["Metrics"]["UnblendedCost"]["Amount"])})
...
def check_top_cost_drivers(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        detail_lines = " · ".join(
            f"{s['service'].replace('Amazon ','').replace('AWS ','')} {_fmt(s['cost'])}" for s in top5
        )
        return Insight(
...
def check_cost_trend(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        return Insight(
            category=CATEGORY_COST, title="Cost trend", value="unavailable", status=STATUS_INFO, detail=str(e)
        )
...
def check_cost_by_region(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        detail_parts = " · ".join(
            f"{r} {_fmt(c)}" for r, c in sorted(regions.items(), key=lambda x: -x[1])[:5]
        )
        return Insight(
            category=CATEGORY_COST,
...
def check_cost_anomalies(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        resp = ce.get_anomalies(DateInterval={"StartDate": str(start), "EndDate": str(end)}, MaxResults=10)
...
def check_cost_by_env_tag(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        return Insight(
            ...
            value=f"{untagged_pct}% untagged ({primary_key})"
            if untagged_pct > 10
            else detail_parts[:60] or "All tagged",
            status=status,
...
def check_nat_gateway_cost(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        nat_cost = _ce_total(
            ...
            str(start),
            str(end),
            filters={"Dimensions": {"Key": "SERVICE", "Values": ["Amazon Virtual Private Cloud"]}},
        )
...
def check_vpc_endpoints_missing(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        resp = ec2.describe_vpc_endpoints(Filters=[{"Name": "state", "Values": ["available", "pending"]}])
        existing = set()
        ...
        if not missing:
            return Insight(
                category=CATEGORY_NETWORKING,
                title="VPC Endpoints for key services",
                value="All present",
                status=STATUS_OK,
        ...
        missing_str = (
            ", ".join(sorted(missing)).upper().replace("ECR.DKR", "ECR").replace("DYNAMODB", "DynamoDB")
        )
        return Insight(
            category=CATEGORY_NETWORKING,
...
def check_savings_plans_coverage(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        start = end - timedelta(days=30)
        resp = ce.get_savings_plans_utilization(TimePeriod={"Start": str(start), "End": str(end)})
        ...
        # Get coverage
        cov_resp = ce.get_savings_plans_coverage(TimePeriod={"Start": str(start), "End": str(end)})
        ...
        return Insight(
            ...
            context={
                ...
                "uncovered_usd": round(uncovered_cost, 2),
            },
        )
    except Exception as e:
...
def check_ondemand_vs_committed(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        return Insight(
            ...
            context={
                "on_demand_usd": round(on_demand, 2),
...
def check_ec2_rightsizing(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        total_savings = sum(
            float(
                r.get("ModifyRecommendationDetail", r.get("TerminateRecommendationDetail", {})).get(
...
def check_cloudwatch_logs_top_groups(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        detail_lines = " · ".join(f"{g['name'].split('/')[-1]} {g['size_gb']:.1f}GB" for g in top5)
        return Insight(
            category=CATEGORY_OBSERVABILITY,
        ...
        return Insight(
            category=CATEGORY_OBSERVABILITY,
            title="Top CloudWatch Log Groups",
            value="unavailable",
            status=STATUS_INFO,
            detail=str(e),
        )
    ...
    try:
        ...
        if not no_lifecycle:
            return Insight(
                ...
                value="All have lifecycle rules",
                status=STATUS_OK,
        ...
        return Insight(
            category=CATEGORY_STORAGE,
            title="S3 without lifecycle rules",
            value="unavailable",
            status=STATUS_INFO,
            detail=str(e),
        )
...
def check_rds_multiaz_nonprod(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        for reg in regions:
            ...
            for inst in resp.get("DBInstances", []):
                if not inst.get("MultiAZ"):
                    continue
                tags = {t["Key"].lower(): t["Value"].lower() for t in inst.get("TagList", [])}
        ...
        if not suspects:
            return Insight(
                category=CATEGORY_STORAGE,
                title="RDS Multi-AZ in non-prod",
                value="None found",
                status=STATUS_OK,
...
def check_lambda_deprecated_runtimes(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        if not deprecated:
            return Insight(
                category=CATEGORY_COMPUTE,
                title="Lambda deprecated runtimes",
...
def check_ebs_provisioned_iops_detached(aws: AWSConfig, ls: LocalStackConfig) -> Insight:
    ...
    try:
        ...
        detail_lines = " · ".join(
            f"{v['name']} ({v['type']}, {v['iops']:,} IOPS) {_fmt(v['monthly_cost'])}/mo" for v in top[:5]
        )
        return Insight(
            category=CATEGORY_STORAGE,
    ...
    try:
            ...
            for page in paginator.paginate(
                ...
                for v in page.get("Volumes", []):
                    ...
                    try:
                        ...
                        utilization_pct = (
                            round(avg_iops / provisioned_iops * 100, 1) if provisioned_iops else 0
                        )
        ...
        return Insight(
            ...
            context={
                ...
                "total_iops_wasted": total_iops_wasted,
            },
        )
    except Exception as e:
...
def run_all_insights(aws: AWSConfig, ls: LocalStackConfig, progress_cb=None) -> list[Insight]:
