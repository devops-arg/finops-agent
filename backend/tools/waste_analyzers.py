...
CACHE_MONTHLY_COST = {
    ...
    "cache.m5.xlarge": 192,
    "cache.m6g.large": 82,
    "cache.m6g.xlarge": 164,
    "cache.r5.large": 148,
    "cache.r5.xlarge": 296,
    "cache.r6g.large": 125,
    "cache.r6g.xlarge": 250,
}
...
class BaseAnalyzer(ABC):
    ...
    def _cw_avg(
        self, namespace: str, metric: str, dimensions: list[dict], days: int = 7, stat: str = "Average"
    ) -> float:
...
class EBSUnattachedAnalyzer(BaseAnalyzer):
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "vol-0abc1234567890001",
                "ebs_volume",
                self.service,
                self.category,
                "EBS volume unattached for 45 days",
                "200GB gp2 volume not attached. $20/mo.",
                SEVERITY_INFO,
                20.0,
                20.0,
                "us-east-1a",
                ...
                metadata={"size_gb": 200, "volume_type": "gp2"},
            ),
            Finding(
                "vol-0abc1234567890002",
                "ebs_volume",
                self.service,
                self.category,
                "EBS volume unattached for 90 days",
                "500GB gp2 volume not attached. $50/mo.",
                SEVERITY_WARNING,
                50.0,
                50.0,
                "us-east-1b",
                idle_days=90,
                tags={"env": "dev"},
                metadata={"size_gb": 500, "volume_type": "gp2"},
            ),
            Finding(
                "vol-0abc1234567890003",
                "ebs_volume",
                self.service,
                self.category,
                "EBS volume unattached for 120 days",
                "2000GB gp2 volume. $200/mo.",
                SEVERITY_CRITICAL,
                200.0,
                200.0,
                "us-east-1a",
                ...
                metadata={"size_gb": 2000, "volume_type": "gp2"},
            ),
        ]
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "EC2 stopped 30d with 2 EBS volumes",
                "t3.large stopped, 2 EBS volumes attached. $80/mo.",
                SEVERITY_WARNING,
                80.0,
                80.0,
                "us-east-1",
            ...
            Finding(
                ...
                self.service,
                self.category,
                "EC2 stopped 60d with 3 EBS volumes",
                "m5.xlarge dev machine stopped 60d. 3 EBS volumes, $120/mo.",
                SEVERITY_WARNING,
                120.0,
                120.0,
                "us-east-1",
            ...
            Finding(
                ...
                self.service,
                self.category,
                "EC2 stopped 90d with 1 EBS volume",
                "c5.2xlarge staging load gen stopped 90d. $248/mo waste.",
                SEVERITY_CRITICAL,
                248.0,
                248.0,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "EC2 missing required tags (env, team)",
                "m5.xlarge running without cost allocation tags. Cannot attribute spend.",
                SEVERITY_INFO,
                140.0,
                0.0,
                "us-east-1",
            ...
            Finding(
                ...
                self.category,
                "EC2 missing required tags (env, team)",
                "c5.2xlarge running without tags. $248/mo unattributed.",
                SEVERITY_INFO,
                248.0,
                0.0,
                "us-east-1",
            ...
            Finding(
                ...
                self.category,
                "EC2 missing required tags (env, team)",
                "r5.xlarge (RDS-like workload?) running without tags.",
                SEVERITY_INFO,
                182.0,
                0.0,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "AMI with no running instances",
                "ami-bastion-v3 not in use. $4/mo in snapshots.",
                SEVERITY_INFO,
                4.0,
                4.0,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "Security group not attached to any resource",
                "'sg-old-bastion-2022' has no ENIs. Stale from decommissioned bastion.",
                SEVERITY_INFO,
                0.0,
                0.0,
                "us-east-1",
            ...
            Finding(
                ...
                self.category,
                "Security group not attached to any resource",
                "'sg-migration-workers' has no ENIs. Migration completed Q4 2024.",
                SEVERITY_INFO,
                0.0,
                0.0,
                "us-east-1",
            ...
            Finding(
                ...
                self.category,
                "Security group not attached to any resource",
                "'sg-lb-old-api-v1' has no ENIs. ALB replaced.",
                SEVERITY_INFO,
                0.0,
                0.0,
                "us-east-1",
            ...
            Finding(
                ...
                self.category,
                "Security group not attached to any resource",
                "'sg-rds-staging-v1' has no ENIs. Staging DB replaced.",
                SEVERITY_INFO,
                0.0,
                0.0,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "Snapshot — source volume deleted",
                "500GB snapshot of deleted volume. $25/mo.",
                SEVERITY_WARNING,
                25.0,
                25.0,
                "us-east-1",
            ...
            Finding(
                ...
                self.category,
                "Snapshot — source volume deleted",
                "100GB snapshot of deleted volume. $5/mo.",
                SEVERITY_INFO,
                5.0,
                5.0,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "Snapshot older than 90 days (180d)",
                "1000GB old snapshot. $50/mo.",
                SEVERITY_WARNING,
                50.0,
                50.0,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                idle_days=75,
                tags={"team": "payments"},
                metadata={"size_gb": 800, "description": "Created by CreateImage for ami-0payments-v4"},
            ),
            Finding(
                "snap-0ami002",
                "ebs_snapshot",
                ...
                idle_days=120,
                tags={"team": "platform"},
                metadata={"size_gb": 400, "description": "Created by CreateImage for ami-0worker-v2"},
            ),
        ]
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                metadata={"instance_class": "db.t3.medium", "storage_gb": 100},
            ),
            Finding(
                "db-sandbox-payments",
                "rds_instance",
                ...
                23.0,
                "us-east-1",
                tags={"env": "sandbox", "team": "payments"},
                metadata={"instance_class": "db.t3.large", "storage_gb": 200},
            ),
        ]
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                "us-east-1",
                tags={"env": "staging"},
                metadata={"instance_class": "db.t3.medium", "env": "staging"},
            ),
            Finding(
                "db-growth-staging",
                "rds_instance",
                ...
                metadata={"instance_class": "db.m5.large", "env": "staging"},
            ),
        ]
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                idle_days=65,
                tags={"team": "platform"},
                metadata={"size_gb": 500, "db": "ribbon-prod-postgres"},
            ),
            Finding(
                "rds-snap-old-002",
                "rds_snapshot",
                ...
                idle_days=120,
                tags={"team": "platform"},
                metadata={"size_gb": 200, "db": "ribbon-staging-postgres"},
            ),
        ]
...
class ELBNoTargetsAnalyzer(BaseAnalyzer):
    ...
    def _live(self) -> list[Finding]:
        ...
        for lb in lbs:
            ...
            all_unhealthy = True
            for tg in tgs:
                health = elb.describe_target_health(TargetGroupArn=tg["TargetGroupArn"]).get(
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                "'ribbon-prod-logs' has 1.2TB Standard. $110/mo → $25/mo with Glacier.",
                SEVERITY_WARNING,
                110.0,
                85.0,
                "us-east-1",
            ...
            Finding(
                ...
                self.category,
                "S3 bucket without lifecycle policy",
                "'ribbon-staging' 70GB Standard no lifecycle.",
                SEVERITY_INFO,
                11.0,
                6.0,
                "us-east-1",
    ...
    def _live(self) -> list[Finding]:
        ...
        def _check(s3, name):
            try:
                ...
                stale = [
                    u for u in uploads if u.get("Initiated", datetime.utcnow()).replace(tzinfo=None) < cutoff
                ]
                if not stale:
                    return None
                ...
                return Finding(
                    ...
                    estimated_savings_usd=cost,
                    region="us-east-1",
                    metadata={"bucket": name, "stale_uploads": len(stale)},
                )
            except Exception:
                return None
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "3 stale incomplete multipart uploads",
                "'ribbon-prod-assets' hidden storage. $7.50/mo.",
                SEVERITY_INFO,
                7.5,
                7.5,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "Versioning on but no lifecycle for old versions",
                "'ribbon-prod-data' accumulating versions. $30/mo extra.",
                SEVERITY_WARNING,
                30.0,
                20.0,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        ...
        return [
            Finding(
                ...
                f"'{name}' stored forever. ${cost}/mo.",
                _severity_from_savings(cost),
                cost,
                savings,
                "us-east-1",
...
class CloudWatchLogsEmptyAnalyzer(BaseAnalyzer):
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "/aws/lambda/ribbon-old-job",
                "log_group",
                self.service,
                self.category,
                "Log group empty for 30+ days",
                "Empty log group from deleted Lambda.",
                SEVERITY_INFO,
                0.0,
                0.0,
                "us-east-1",
...
class CloudWatchLogsOrphanLambdaAnalyzer(BaseAnalyzer):
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "/aws/lambda/ribbon-deprecated-cron",
                "log_group",
                self.service,
                self.category,
                "Lambda log group for deleted function",
                "'ribbon-deprecated-cron' deleted, logs remain.",
                SEVERITY_INFO,
                0.5,
                0.5,
                "us-east-1",
...
class DynamoDBNoBackupAnalyzer(BaseAnalyzer):
    ...
    def _live(self) -> list[Finding]:
        ...
        for table in tables:
            try:
                ...
                pitr = backup.get("ContinuousBackupsDescription", {}).get(
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "ribbon-payments-events",
                "dynamodb_table",
                self.service,
                self.category,
                "DynamoDB table without PITR backup",
                "'ribbon-payments-events' has no backup. Data risk.",
                SEVERITY_WARNING,
                0.0,
                0.0,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                tags={"env": "staging", "team": "growth"},
                metadata={
                    "domain": "d1abcdefghijk.cloudfront.net",
                    "origins": 1,
                    "comment": "ribbon-staging-frontend-old",
                },
            ),
        ]
        ...
        for inst in resp.get("DBInstances", []):
            ...
            findings.append(
                Finding(
                    ...
                    service=self.service,
                    category=self.category,
                    title="RDS idle — 0 connections in last 7 days",
...
class RDSOversizedAnalyzer(BaseAnalyzer):
    ...
    def _live(self) -> list[Finding]:
        ...
        for inst in rds.describe_db_instances().get("DBInstances", []):
            ...
            db_id = inst["DBInstanceIdentifier"]
            cpu = self._cw_avg(
                "AWS/RDS", "CPUUtilization", [{"Name": "DBInstanceIdentifier", "Value": db_id}], days=7
            )
            free_mem = self._cw_avg(
                "AWS/RDS", "FreeableMemory", [{"Name": "DBInstanceIdentifier", "Value": db_id}], days=7
            )
    ...
    def _live(self) -> list[Finding]:
        ...
        for res in resp.get("Reservations", []):
            for inst in res.get("Instances", []):
                iid = inst["InstanceId"]
                cpu = self._cw_avg(
                    "AWS/EC2", "CPUUtilization", [{"Name": "InstanceId", "Value": iid}], days=7
                )
                if cpu == 0 or cpu >= 10:
                    continue
                itype = inst.get("InstanceType", "unknown")
    ...
    def _mock(self) -> list[Finding]:
        ...
        return [
            Finding(
                ...
                "us-east-1",
                tags={"env": "staging"},
                metadata={"instance_type": itype, "cpu_avg_7d": cpu, "name": name},
            )
...
class ElastiCacheIdleAnalyzer(BaseAnalyzer):
    ...
    def _live(self) -> list[Finding]:
        ...
        for cluster in ec.describe_cache_clusters().get("CacheClusters", []):
            ...
            conns = self._cw_avg(
                "AWS/ElastiCache", "CurrConnections", [{"Name": "CacheClusterId", "Value": cid}], days=14
            )
            if conns > 0:
                continue
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                metadata={"node_type": "cache.m5.large", "engine": "redis"},
            ),
            Finding(
                "ribbon-loadtest-memcache",
                "elasticache_cluster",
                self.service,
                self.category,
                ...
                idle_days=21,
                tags={"env": "staging", "team": "growth"},
                metadata={"node_type": "cache.r5.large", "engine": "memcached"},
            ),
        ]
...
class ElastiCacheOversizedAnalyzer(BaseAnalyzer):
    ...
    def _live(self) -> list[Finding]:
        ...
        for cluster in ec.describe_cache_clusters().get("CacheClusters", []):
            cid = cluster["CacheClusterId"]
            free_mem = self._cw_avg(
                "AWS/ElastiCache", "FreeableMemory", [{"Name": "CacheClusterId", "Value": cid}], days=7
            )
            if free_mem == 0:
                continue
            node_type = cluster.get("CacheNodeType", "cache.t3.micro")
            total_mem_map = {
                "cache.t3.micro": 512,
                "cache.m5.large": 6742,
                "cache.m6g.large": 6742,
                "cache.r5.large": 13600,
                "cache.r6g.large": 13600,
            }
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "ElastiCache oversized — 84% memory free",
                "cache.r6g.large, 84% free. Downsize saves $50/mo.",
                SEVERITY_WARNING,
                125.0,
                50.0,
                "us-east-1",
    ...
    def _live(self) -> list[Finding]:
        ...
        for rg in ec.describe_replication_groups().get("ReplicationGroups", []):
            ...
            for replica in replicas:
                gets = self._cw_avg(
                    "AWS/ElastiCache", "GetTypeCmds", [{"Name": "CacheClusterId", "Value": replica}], days=7
                )
                if gets > 0:
                    all_idle = False
                    break
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                metadata={
                    "replica_count": 2,
                    "recommendation": "Remove read replicas or configure app read endpoint",
                },
            ),
        ]
...
class LambdaUnusedAnalyzer(BaseAnalyzer):
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "ribbon-old-migration-job",
                "lambda_function",
                self.service,
                self.category,
                "Lambda — 0 invocations in 90 days",
                "'ribbon-old-migration-job' not invoked 90d. One-time migration completed Q1.",
                SEVERITY_INFO,
                0.5,
                0.5,
                "us-east-1",
            ...
            Finding(
                ...
                self.category,
                "Lambda — 0 invocations in 90 days",
                "'ribbon-test-webhook' unused 90d. Dev test function left deployed.",
                SEVERITY_INFO,
                0.1,
                0.1,
                "us-east-1",
                idle_days=90,
                tags={"env": "dev"},
                metadata={"memory_mb": 128, "runtime": "nodejs18.x"},
            ),
            Finding(
                "ribbon-sms-fallback-v1",
                "lambda_function",
                self.service,
                self.category,
                "Lambda — 0 invocations in 90 days",
                "'ribbon-sms-fallback-v1' unused 90d. Replaced by v2 with SNS.",
                SEVERITY_INFO,
                0.3,
                0.3,
                "us-east-1",
                idle_days=90,
                tags={"team": "payments"},
                metadata={"memory_mb": 256, "runtime": "python3.11"},
            ),
            Finding(
                "ribbon-report-scheduler-cron",
                "lambda_function",
                self.service,
                self.category,
                "Lambda — 0 invocations in 90 days",
                "'ribbon-report-scheduler-cron' unused 90d. Reports now generated by ECS batch job.",
                SEVERITY_INFO,
                0.2,
                0.2,
                "us-east-1",
                idle_days=90,
                tags={"team": "data"},
                metadata={"memory_mb": 256, "runtime": "python3.10"},
            ),
        ]
...
class LambdaHighErrorRateAnalyzer(BaseAnalyzer):
    ...
    def _live(self) -> list[Finding]:
        ...
        for func in lam.list_functions().get("Functions", []):
            name = func["FunctionName"]
            errors = self._cw_avg(
                "AWS/Lambda", "Errors", [{"Name": "FunctionName", "Value": name}], days=7, stat="Sum"
            )
            invocations = self._cw_avg(
                "AWS/Lambda", "Invocations", [{"Name": "FunctionName", "Value": name}], days=7, stat="Sum"
            )
        ...
        return [
            ...
            Finding(
                ...
                metadata={
                    ...
                    "errors_7d": 580,
                    "invocations_7d": 1000,
                    "error_type": "DynamoDB ProvisionedThroughputExceededException",
                },
            ),
        ]
...
class NATLowTrafficAnalyzer(BaseAnalyzer):
    ...
    def _live(self) -> list[Finding]:
        ec2 = self._client("ec2")
        findings = []
        nats = ec2.describe_nat_gateways(Filters=[{"Name": "state", "Values": ["available"]}]).get(
            "NatGateways", []
        )
        for nat in nats:
            nat_id = nat["NatGatewayId"]
            bytes_out = self._cw_avg(
                "AWS/NATGateway",
            ...
            title = (
                "NAT Gateway idle — 0 traffic for 7+ days"
                if is_dead
                else f"NAT Gateway low traffic — {gb_per_day:.2f}GB/day avg"
            )
            description = (
                f"NAT {nat_id} has had zero traffic for 7+ days. ${cost}/mo wasted. Safe to delete."
                if is_dead
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "NAT Gateway low traffic — 0.12GB/day avg",
                "NAT in staging, 0.12GB/day. $32/mo.",
                SEVERITY_WARNING,
                32.85,
                19.0,
                "us-east-1",
        ...
        for az, nat_list in az_nats.items():
            ...
            findings.append(
                Finding(
                    ...
                    metadata={
                        "az": az,
                        "nat_count": len(nat_list),
                        "nat_ids": [n["NatGatewayId"] for n in nat_list],
                    },
                )
            )
        return findings
    ...
    def _mock(self) -> list[Finding]:
        ...
        return [
            Finding(
                ...
                cost_each,
                "us-east-1",
                metadata={"az": "us-east-1a", "nat_count": 2, "nat_ids": ["nat-0prod001", "nat-0prod002"]},
            ),
        ]
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "NAT traffic to S3 — VPC Endpoint opportunity",
                "60GB/week S3 traffic via NAT. Add endpoint saves $97/mo.",
                SEVERITY_WARNING,
                194.0,
                97.0,
                "us-east-1",
...
class NATCrossAZAnalyzer(BaseAnalyzer):
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "nat-crossaz-us-east-1",
                "nat_gateway",
                self.service,
                self.category,
                "Cross-AZ traffic through NAT Gateway — avoidable",
                "Instances in us-east-1b routing via NAT in us-east-1a. $142/mo cross-AZ.",
                SEVERITY_WARNING,
                142.0,
                100.0,
                "us-east-1",
...
class EBSNoIOPSAnalyzer(BaseAnalyzer):
    ...
    def _live(self) -> list[Finding]:
        ...
        for vol in vols:
            vid = vol["VolumeId"]
            reads = self._cw_avg(
                "AWS/EBS", "VolumeReadOps", [{"Name": "VolumeId", "Value": vid}], days=7, stat="Sum"
            )
            writes = self._cw_avg(
                "AWS/EBS", "VolumeWriteOps", [{"Name": "VolumeId", "Value": vid}], days=7, stat="Sum"
            )
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "EBS volume — 0 IOPS in last 7 days",
                "300GB gp2, no I/O 7d. $30/mo.",
                SEVERITY_WARNING,
                30.0,
                30.0,
                "us-east-1",
...
class DynamoDBIdleAnalyzer(BaseAnalyzer):
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "ribbon-feature-flags-v1",
                "dynamodb_table",
                self.service,
                self.category,
                "DynamoDB table idle 30 days",
                "'ribbon-feature-flags-v1' replaced by v2, idle. $12/mo.",
                SEVERITY_INFO,
                12.0,
                12.0,
                "us-east-1",
...
class DynamoDBGSIUnusedAnalyzer(BaseAnalyzer):
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "ribbon-payments-events/gsi-status-createdAt",
                "dynamodb_gsi",
                self.service,
                self.category,
            ...
            Finding(
                "ribbon-users/gsi-email-index-old",
                "dynamodb_gsi",
                self.service,
                self.category,
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "DynamoDB PROVISIONED — PAY_PER_REQUEST cheaper",
                "'ribbon-sessions' provisioned $47/mo vs on-demand $18/mo.",
                SEVERITY_WARNING,
                47.0,
                29.0,
                "us-east-1",
...
class S3EmptyBucketAnalyzer(BaseAnalyzer):
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "ribbon-test-uploads-2024",
                "s3_bucket",
                self.service,
                self.category,
                "S3 bucket empty",
                "'ribbon-test-uploads-2024' has 0 bytes.",
                SEVERITY_INFO,
                0.0,
                0.0,
                "us-east-1",
...
class Route53IdleAnalyzer(BaseAnalyzer):
    ...
    def _live(self) -> list[Finding]:
        ...
        for zone in zones:
            ...
            queries = self._cw_avg(
                "AWS/Route53", "DNSQueries", [{"Name": "HostedZoneId", "Value": zone_id}], days=30, stat="Sum"
            )
...
class SQSIdleQueueAnalyzer(BaseAnalyzer):
    ...
    def _live(self) -> list[Finding]:
        ...
        for url in queues:
            name = url.split("/")[-1]
            msgs = self._cw_avg(
                "AWS/SQS", "NumberOfMessagesSent", [{"Name": "QueueName", "Value": name}], days=30, stat="Sum"
            )
            if msgs and msgs > 0:
                continue
            findings.append(
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "ribbon-old-notifications-dlq",
                "sqs_queue",
                self.service,
                self.category,
                "SQS queue — 0 messages in 30 days",
                "'ribbon-old-notifications-dlq' idle 30d.",
                SEVERITY_INFO,
                0.0,
                0.0,
                "us-east-1",
...
class SNSIdleTopicAnalyzer(BaseAnalyzer):
    ...
    def _live(self) -> list[Finding]:
        ...
        for topic in sns.list_topics().get("Topics", []):
            arn = topic["TopicArn"]
            name = arn.split(":")[-1]
            published = self._cw_avg(
                "AWS/SNS",
                "NumberOfMessagesPublished",
                [{"Name": "TopicName", "Value": name}],
                days=30,
                stat="Sum",
            )
    ...
    def _mock(self) -> list[Finding]:
        ...
        return [
            Finding(
                f"{arn_base}:ribbon-fraud-alerts-v1",
                "sns_topic",
                self.service,
                self.category,
                ...
                idle_days=45,
                tags={"team": "payments"},
                metadata={"topic_name": "ribbon-fraud-alerts-v1"},
            ),
            Finding(
                f"{arn_base}:ribbon-staging-deploys",
                "sns_topic",
                self.service,
                self.category,
                "SNS topic — 0 messages in 30 days",
                ...
                idle_days=30,
                tags={"env": "staging"},
                metadata={"topic_name": "ribbon-staging-deploys"},
            ),
        ]
...
class SecretsManagerUnusedAnalyzer(BaseAnalyzer):
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "arn:aws:secretsmanager:us-east-1:123:secret:ribbon/old-api-key",
                "secret",
                self.service,
                self.category,
                ...
                idle_days=120,
                tags={"team": "platform"},
                metadata={"secret_name": "ribbon/old-api-key"},
            ),
            Finding(
                "arn:aws:secretsmanager:us-east-1:123:secret:ribbon/staging-db-pass-v1",
                "secret",
                self.service,
                self.category,
                ...
                idle_days=95,
                tags={"env": "staging"},
                metadata={"secret_name": "ribbon/staging-db-pass-v1"},
            ),
            Finding(
                "arn:aws:secretsmanager:us-east-1:123:secret:ribbon/sendgrid-test",
                "secret",
                self.service,
                self.category,
                ...
                idle_days=180,
                tags={"env": "dev"},
                metadata={"secret_name": "ribbon/sendgrid-test"},
            ),
        ]
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "ECR repository — no pulls in 180 days",
                "'ribbon-old-worker' 180d no pulls. $2/mo.",
                SEVERITY_INFO,
                2.0,
                2.0,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "EC2 Graviton: m5.xlarge → m6g.xlarge",
                "4 EKS nodes m5.xlarge → m6g.xlarge. Save $112/mo total.",
                SEVERITY_WARNING,
                560.0,
                112.0,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "EBS gp2 → gp3 migration (2870GB)",
                "2870GB gp2 $287/mo → gp3 $229/mo. Save $57/mo.",
                SEVERITY_WARNING,
                287.0,
                57.0,
                "us-east-1",
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                ...
                self.service,
                self.category,
                "ElastiCache old gen: cache.r5.large → cache.r6g.large",
                "Migrate to Graviton. Save $22/mo.",
                SEVERITY_INFO,
                148.0,
                22.0,
                "us-east-1",
...
class LambdaDeprecatedRuntimeAnalyzer(BaseAnalyzer):
    ...
    def _mock(self) -> list[Finding]:
        return [
            Finding(
                "ribbon-email-service",
                "lambda_function",
                self.service,
                self.category,
                "Lambda deprecated runtime: python3.8",
                "'ribbon-email-service' on Python 3.8 (EOL). Upgrade now.",
                SEVERITY_CRITICAL,
                1.0,
                0.0,
                "us-east-1",
            ...
            Finding(
                "ribbon-legacy-webhook",
                "lambda_function",
                self.service,
                self.category,
                "Lambda deprecated runtime: nodejs14.x",
                "'ribbon-legacy-webhook' on Node.js 14 (EOL).",
                SEVERITY_CRITICAL,
                1.0,
                0.0,
                "us-east-1",
    ...
    def _live(self) -> list[Finding]:
        ...
        for func in lam.list_functions().get("Functions", []):
            ...
            if mem <= 256:
                continue
            name = func["FunctionName"]
            avg_dur = self._cw_avg(
                "AWS/Lambda", "Duration", [{"Name": "FunctionName", "Value": name}], days=7
            )
            if avg_dur == 0:
                continue
...
class LambdaOrphanLayerAnalyzer(BaseAnalyzer):
        ...
        for layer_arn in all_layers - used_layers:
            ...
            findings.append(
                Finding(
                    ...
                    service=self.service,
                    category=self.category,
                    title="Lambda layer not used by any function",
    ...
    def _mock(self) -> list[Finding]:
        ...
        return [
            Finding(
                f"{arn_base}:ribbon-common-utils",
                "lambda_layer",
                self.service,
                self.category,
    ...
    for region in scan_regions:
        ...
        for i, cls in enumerate(regional_classes, 1):
            ...
            try:
                ...
                all_findings.extend(findings)
                logger.info(
                    f"[{region}] ({i}/{len(regional_classes)}) {cls.__name__} → {len(findings)} findings"
                )
                if findings_cb and findings:
