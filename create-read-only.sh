...
#
# Usage:
#   ./create-read-only.sh [parent-profile]
#
# What it does:
#   1. Uses parent profile (default: sandbox-production) to create an IAM user
#      named "finops-agent-readonly" with AWS managed ReadOnlyAccess policy.
...
#   5. Tests the new creds and verifies they CANNOT write (tries s3 mb, expects fail).
#
# The created user is strictly read-only (ReadOnlyAccess managed policy includes
# ce:* for Cost Explorer, ec2/rds/eks/elasticache/s3/cloudwatch Describe/List/Get).

set -euo pipefail

PARENT_PROFILE="${1:-sandbox-production}"
...
if aws iam get-user --user-name "$NEW_USER" --profile "$PARENT_PROFILE" >/dev/null 2>&1; then
  ...
  aws iam create-user --user-name "$NEW_USER" --profile "$PARENT_PROFILE" >/dev/null
...
touch "$ENV_FILE"
TMP_ENV=$(mktemp)
# Keep lines that are NOT AWS_* or USE_LOCALSTACK
...
# Remove existing [finops] section then append
TMP_CRED=$(mktemp)
...
# ── 8. Verify user CANNOT write (safety check) ──────────────────────────
info "Verifying WRITE is blocked (trying s3 mb, expecting failure)..."
TEST_BUCKET="finops-agent-delete-me-$(date +%s)"
if aws s3 mb "s3://$TEST_BUCKET" --profile "$NEW_PROFILE" 2>/dev/null; then
  # Cleanup if somehow it succeeded
  aws s3 rb "s3://$TEST_BUCKET" --profile "$NEW_PROFILE" 2>/dev/null || true
  fail "SECURITY ISSUE: user was able to create a bucket! Check policies."
else
  ok "Write blocked as expected (AccessDenied / 403)"
fi
